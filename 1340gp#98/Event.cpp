...
bool Travel(Player* player, Planet* current_planet, Planet* destination_planet) {
    ...
    int FoodPlayer, WaterPlayer, FuelPlayer;
    FoodPlayer = getCargoQuantity(player, "food");
    WaterPlayer = getCargoQuantity(player, "water");
    FuelPlayer = getCargoQuantity(player, "fuel");
    if (FoodPlayer < food_consumption | WaterPlayer < water_consumption | FuelPlayer < fuel_consumption ){
        std::cout << "You cargo is not enough for travelling.\n"; 
        return false;
    }
    else{
